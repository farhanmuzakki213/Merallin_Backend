                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="relative z-20 bg-transparent">
                        <select
                            ...
                            @change="perPage = parseInt($event.target.value)">
                    ...
                    <template x-for="person in paginatedData" :key="person.id">
                         <div class="grid grid-cols-12 border-t border-gray-100 dark:border-gray-800">
                            <div class="col-span-3 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <div>
                                    <p class="text-theme-sm block font-medium text-gray-800 dark:text-white/90" x-text="person.name"></p>
                            ...
                            <div class="col-span-1 flex items-center border-r border-gray-100 px-4 py-3 dark:border-gray-800">
                                <p :class="{ 'bg-success-50 dark:bg-success-500/15 text-success-700 dark:text-success-500': person.status === 'Hired', 'bg-warning-50 dark:bg-warning-500/15 text-warning-700 dark:text-orange-400': person.status === 'In Progress', 'bg-error-50 dark:bg-error-500/15 text-error-700 dark:text-error-500': person.status === 'Pending' }" class="text-theme-xs rounded-full px-2 py-0.5 font-medium" x-text="person.status"></p>
                            </div>
                            <div class="col-span-1 flex items-center px-4 py-3">
            ...
            <div class="border-t border-gray-100 py-4 pr-4 pl-[18px] dark:border-gray-800">
                <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between">
                    <p class="border-b border-gray-100 pb-3 text-center text-sm font-medium text-gray-500 xl:border-b-0 xl:pb-0 xl:text-left dark:border-gray-800 dark:text-gray-400">
                        Showing <span x-text="startEntry"></span> to <span x-text="endEntry"></span> of <span x-text="totalEntries"></span> entries
                    </p>
                    <div class="flex items-center justify-center gap-0.5 pt-3 xl:justify-end xl:pt-0">
                        <button @click="prevPage()" class="shadow-theme-xs mr-2.5 flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]" :disabled="currentPage === 1">Previous</button>
                        <template x-for="page in pagesAroundCurrent" :key="page">
                            <button @click="goToPage(page)" :class="currentPage === page ? 'bg-blue-500/[0.08] text-brand-500' : 'text-gray-700 dark:text-gray-400'" class="flex h-10 w-10 items-center justify-center rounded-lg text-sm font-medium hover:bg-blue-500/[0.08]">
                                <span x-text="page"></span>
                            </button>
                        </template>
                        <button @click="nextPage()" class="shadow-theme-xs ml-2.5 flex items-center justify-center rounded-lg border border-gray-300 bg-white px-3.5 py-2.5 text-gray-700 hover:bg-gray-50 disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03]" :disabled="currentPage === totalPages">Next</button>
                    </div>
                </div>
            </div>
        </div>
